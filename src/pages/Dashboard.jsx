...
export default function Dashboard() {
  ...
  const saveProduct = async () => {
    ...
    const productData = {
      ...
      capabilities, limitations, specs, onSale, discount: Number(discount),
      featured: isFeatured, images: finalImagesArray,
    };
    ...
    if (editingId) {
      ...
      result = await supabase.from("products").insert([{ ...productData, stock: 1, images: finalImagesArray }]);
    }
          ...
          {showForm && (
            <GlassCard className="p-8 mt-8 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input type="number" placeholder="Initial Stock" value={stock} onChange={(e) => setStock(e.target.value)}
                  className="w-full p-4 rounded-xl bg-white/5 border border-white/10 focus:border-fuchsia-500 outline-none transition" min="0" />
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-end">
                  <label className="text-xs font-bold text-fuchsia-500 uppercase tracking-widest">Marketing Description</label>
                ...
                <div className="bg-white/5 rounded-xl overflow-auto border border-white/10 min-h-[200px]">
                  {showPreview ? (
                    <div className="p-4 prose prose-invert prose-fuchsia max-w-none min-w-0 w-full">
                      {description
                        ? <div dangerouslySetInnerHTML={{ __html: description.replace(/&nbsp;/g, ' ').replace(/\u00A0/g, ' ') }} />
                        : <p className="text-gray-500 italic">Nothing to preview yet...</p>}
                    </div>
                  ) : (
                    <ReactQuill theme="snow" value={description} onChange={setDescription} className="text-white bg-black/20" />
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <label className="text-xs font-bold text-emerald-500 uppercase tracking-widest">Capabilities</label>
                {capabilities.map((cap, index) => (
                  <input key={index} value={cap} onChange={(e) => handleArrayChange(index, e.target.value, 'cap')}
                ...
                <button type="button" onClick={() => addField('cap')} className="text-xs text-emerald-400 hover:text-emerald-300 font-bold">+ Add Capability</button>
              </div>
              ...
              <div className="space-y-3 p-4 bg-black/20 rounded-xl border border-white/5">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input type="text" placeholder="Key (e.g. RAM)" value={specKey} onChange={(e) => setSpecKey(e.target.value)}
                  ...
                  <input type="text" placeholder="Value (e.g. 16GB)" value={specValue} onChange={(e) => setSpecValue(e.target.value)}
                    ...
                    {existingImages.map((img, idx) => (
                      <div key={idx} className="relative w-20 h-20">
                        ...
                        <button onClick={() => removeExistingImage(idx)}
                          className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs hover:bg-red-500 shadow-xl">×</button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                {imageFiles.length > 0 && <p className="text-xs text-fuchsia-400">{imageFiles.length} files selected.</p>}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <label className="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/10 cursor-pointer hover:bg-white/10 transition">
                  <input type="checkbox" checked={isFeatured} onChange={(e) => setIsFeatured(e.target.checked)} className="w-5 h-5 accent-fuchsia-500" />
                  <span className="text-sm font-medium">Feature on Home</span>
                </label>
                <label className="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/10 cursor-pointer hover:bg-white/10 transition">
                  <input type="checkbox" checked={onSale} onChange={(e) => setOnSale(e.target.checked)} className="w-5 h-5 accent-fuchsia-500" />
              ...
              {onSale && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  <input type="number" placeholder="e.g. 10" value={discount} onChange={(e) => setDiscount(e.target.value)}
              ...
              <div className="flex gap-4 pt-4 border-t border-white/10">
                {editingId && (
                  <button onClick={cancelEdit} className="px-6 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white transition font-bold">Cancel</button>
                )}
                ...
                {products.map((product) => (
                  <GlassCard key={product.id} className="p-4 flex flex-col gap-4 overflow-hidden">
                    ...
                    <div className="mt-auto border-t border-white/10 pt-4 space-y-4">
                      <div className="flex flex-wrap gap-2">
                        {product.featured && <span className="text-[10px] bg-yellow-500/20 text-yellow-300 px-2 py-1 rounded-md border border-yellow-500/20 uppercase font-bold tracking-wider">Featured</span>}
                        {product.onSale && <span className="text-[10px] bg-blue-500/20 text-blue-300 px-2 py-1 rounded-md border border-blue-500/20 uppercase font-bold tracking-wider">Sale {product.discount}%</span>}
                      </div>
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center bg-white/5 rounded-lg border border-white/10 overflow-hidden">
                          <button onClick={() => updateStock(product.id, product.stock, -1)} className="px-3 py-1 hover:bg-white/10 transition text-white/50 hover:text-white">−</button>
                          <input type="number" value={product.stock} onChange={(e) => setManualStock(product.id, e.target.value)}
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => startEdit(product)} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-all" title="Edit">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                            </svg>
                          </button>
                          <button onClick={() => deleteProduct(product.id, product.images)} className="p-2 rounded-lg bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/20 transition-all" title="Delete">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {activeTab === "orders" && (
        <div className="mt-10">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={fetchOrders} className="text-xs text-fuchsia-400 hover:text-fuchsia-300 font-bold border border-fuchsia-500/20 px-3 py-1 rounded-lg transition">
              ↺ Refresh
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {["pending", "confirmed", "delivered", "cancelled"].map((s) => (
              <GlassCard key={s} className="p-4 text-center">
                <p className="text-2xl font-black text-white">{orders.filter(o => o.status === s).length}</p>
                <p className={`text-xs font-bold uppercase tracking-wider mt-1 ${STATUS_STYLES[s].split(' ')[1]}`}>{s}</p>
              </GlassCard>
            ))}
          </div>
                ...
                <GlassCard key={order.id} className="p-0 overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div>
                        <p className="font-bold text-white">#{order.id} — {order.customer_name}</p>
                        <p className="text-xs text-white/40 mt-0.5">
                          {new Date(order.created_at).toLocaleString()} · {order.customer_city}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center gap-3">
                      <p className="font-bold text-fuchsia-400">₦{order.total.toLocaleString()}</p>
                      <span className={`text-[10px] px-2 py-1 rounded-md border uppercase font-bold tracking-wider ${STATUS_STYLES[order.status]}`}>
                        {order.status}
                      </span>
                    ...
                    <div className="border-t border-white/10 p-5 space-y-5">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-white">{order.customer_address}, {order.customer_city}</p>
                        </div>
                      </div>

                      {/* ITEMS */}
                      <div>
                        <p className="text-xs text-white/40 uppercase font-bold mb-3">Items Ordered</p>
                      ...
                      <div className="bg-white/5 rounded-xl p-4 text-sm space-y-2">
                        ...
                        <div className="flex justify-between text-white font-bold border-t border-white/10 pt-2">
                          <span>Total</span><span className="text-fuchsia-400">₦{order.total.toLocaleString()}</span>
                        </div>
                      </div>
